        ...
        if (isValidURL(domain)) {
            ...
            chrome.storage.local.get(presentDate, function (storedObject) {
                ...
                if (!storedObject[presentDate][domain]) {
                    storedObject[presentDate][domain] = [];
                }
                let lastSession = storedObject[presentDate][domain].slice(-1)[0];
                if (lastSession && lastSession.url === url) {
                    lastSession.timeSpent++;
                    lastSession.endTime = startTime;
                } else {
                    storedObject[presentDate][domain].push({ url, startTime, endTime: startTime, timeSpent: 1, sessions: (lastSession ? lastSession.sessions : 0) + 1 });
                }
                chrome.storage.local.set(storedObject, function () {
                    console.log("Set " + domain + " at " + storedObject[presentDate][domain]);
                    const totalTimeSpent = storedObject[presentDate][domain].reduce((total, session) => total + session.timeSpent, 0);
...
function updateBadge() {
    chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (activeTab) {
        ...
        if (isValidURL(domain)) {
            ...
            let presentDate = getDateString(today);
            chrome.storage.local.get(presentDate, function (storedObject) {
                if (storedObject[presentDate] && storedObject[presentDate][domain]) {
                    const totalTimeSpent = storedObject[presentDate][domain].reduce((total, session) => total + session.timeSpent, 0);
                    chrome.action.setBadgeText({ text: secondsToString(totalTimeSpent, true) });
                } else {
